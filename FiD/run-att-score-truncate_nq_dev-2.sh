...
for file in /data/philhoon-relevance/FiD/open_domain_data/ATTEN_TRUNC_PSG/*.json
do
    name=${file##*/}
    base=${name%.json}
    ...
    echo "CUDA_VISIBLE_DEVICES="$gpu_" python test_reader-slurm.py \
        ...
        --n_context 20 \
        --name "$base" \
        --checkpoint_dir /data/philhoon-relevance/FiD/results/ATTEN_TRUNC_PSG
    "
...
# sbatch --gpus 1 --cpus-per-gpu=8 run-att-score-truncate_nq_dev-2.sh
...
if ((i >= 1 && i <= 10)); then
    ...
    echo "CUDA_VISIBLE_DEVICES="$gpu_" python test_reader-att-slurm.py \
      ...
      --n_context "$i" \
      --name "$attempt" \
      --checkpoint_dir /data/philhoon-relevance/FiD/results/NQ_DPR/DEV_ATT_TOKEN_LAYERS
    "
    CUDA_VISIBLE_DEVICES="$gpu_" python test_reader-att-slurm.py \
    --model_path /data/philhoon-relevance/FiD/pretrained_models/nq_reader_large \
    --eval_data /data/philhoon-relevance/FiD/open_domain_data/NQ/"$dataset" \
    --write_results \
    --write_crossattention_scores \

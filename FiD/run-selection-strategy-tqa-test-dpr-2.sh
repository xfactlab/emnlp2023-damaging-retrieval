...
dataArray=("dev" "train")
#strict_positive_naive_damaging_remove_damage_irrelevant_relevant.json
#strict_positive_naive_damaging_remove_damage_relevant.json
#strict_positive_strict_damaging_remove_damage_relevant.json

for file in /data/philhoon-relevance/FiD/open_domain_data/TQA_TEST_DPR_SELECTION/*.json
do
    if [[ $file == *"strict_positive_"* ]]; then
        name=${file##*/}
        base=${name%.json}
        ...
        echo "CUDA_VISIBLE_DEVICES="$gpu_" python test_reader-slurm.py \
          ...
          --n_context 100 \
          --name "$base" \
          --checkpoint_dir /data/philhoon-relevance/FiD/results/TQA_TEST_DPR_SELECTION/
        "

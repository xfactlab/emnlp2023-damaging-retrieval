...
for file in /data/philhoon-relevance/FiD/open_domain_data/TQA_DEV_CONTRIEVER_SELECTION_NEW/*.json
do
    name=${file##*/}
    base=${name%.json}
    ...
    echo "CUDA_VISIBLE_DEVICES="$gpu_" python test_reader-slurm.py \
        ...
        --n_context 100 \
        --name "$base" \
        --checkpoint_dir /data/philhoon-relevance/FiD/results/TQA_DEV_CONTRIEVER_SELECTION_NEW/
    "

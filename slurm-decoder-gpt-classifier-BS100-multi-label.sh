...
bs=100
lrs=(6e-5)
dataset=DecoderCombinedSinlgeFiveLabelDataset
...
#    sequential-decoder-classifier-batch64X2-lr6e-5-n_layer12-combdata-block_size40
    rn="decoder-seq-classifier-layer$layer-batch${batchsize}Xgr${gr}-lr$lr-combmultilabeldata-blocksize$bs"
    outpath="$path/$rn"
...
#    echo $outpath
    ...
    --with_tracking True \
    --report_to wandb \
    --wandb_project decoder-sequential-classifier-multilabel \
    --run_name "$rn" \
    --output_dir "$outpath" \
...
# sbatch -p rtx6000 --gpus 1 --cpus-per-gpu=8 slurm-decoder-gpt-classifier-BS100-multi-label.sh

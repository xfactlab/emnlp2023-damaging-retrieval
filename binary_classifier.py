...
from src.data import (
    ...
    BinaryCustomDatasetDecisiveBinaryInference,
)
...
DATASET_MAPPING = {
    ...
    'BinaryCustomDatasetDecisiveBinaryInference' : BinaryCustomDatasetDecisiveBinaryInference,
}
...
def main():
    ...
    if train_args.class_weights:
        train_labels = np.array([int(ins['em']) for ins in train_data])
    ...
    #
    # if train_args.output_dir is not None:
    #     with open(os.path.join(train_args.output_dir,
    #                            f"Final_results_epoch{train_args.num_train_epochs}_steps{train_args.max_train_steps}.json"),
    #               "w") as f:
